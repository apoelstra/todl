...
### UNCOMMENT WHATEVER YOU WANT TO PRINT. REDIRECT TO THE APPROPIRATE .tex.inc FILE
# 1.1 Powers of Two
#print_list([("2^{%d}G" % i, 2 ** i) for i in xrange(0, 256)], 9, 20)
# 1.2 Base 4 digits
#print_list([("%d \\times 2^{%d}G" % (j, i * 2), j * 2 ** (i * 2)) for i in xrange(0, 128) for j in xrange(1,4)], 18, 20)
# 1.3 Base 16 digits
#print_list([("%d \\times 2^{%d}G" % (j, i * 4), j * 2 ** (i * 4)) for i in xrange(0, 64) for j in xrange(1,16)], 18, 20)
# 1.4 Base 256 digits
print_list([("%d \\times 2^{%d}G" % (j, i * 8), j * 2 ** (i * 8)) for i in xrange(0, 32) for j in xrange(1,256)], 18, 20)
